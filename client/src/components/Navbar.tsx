...
import { Menu, Search, RefreshCw, Heart, History, X, Film, Link2, Play, AlertTriangle } from 'lucide-react';
...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 h-14 flex items-center justify-between px-4 bg-surface/95 backdrop-blur-sm border-b border-surface-200">
      ...
      <div className="flex-1 flex justify-center max-w-xl mx-auto px-4 relative" ref={dropdownRef}>
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              onKeyDown={e => {
                if (e.key === 'Escape') {
                  clearSearch();
                } else if (e.key === 'Enter' && /^https?:\/\//i.test(localSearch.trim())) {
          ...
          <button
            ...
            className={`p-2 rounded-full border transition-all shrink-0 hover:bg-brand/10 hover:text-brand hover:border-brand/40
                        ${showUrlDropdown
                          ...
                          : 'bg-surface-200 border-surface-300 text-gray-400'
                        }`}
        ...
        {localSearch.trim() && /^https?:\/\//i.test(localSearch.trim()) && (
          <div className="absolute top-full left-4 right-4 mt-2 bg-surface-100/95 backdrop-blur-xl border border-brand/30 rounded-xl p-3 shadow-2xl z-50 animate-fade-in max-w-md mx-auto">
            <button
              onClick={() => {
              ...
              className="w-full text-left flex items-center gap-3 p-2 rounded-lg hover:bg-brand/10 transition-colors text-white text-xs font-semibold"
            >
              <div className="p-1.5 bg-brand text-white rounded-md shrink-0">
                <Play size={12} className="fill-current" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="truncate text-gray-300">Play URL instantly inside player</p>
                <p className="truncate text-[10px] text-brand/70 font-mono mt-0.5">{localSearch.trim()}</p>
              </div>
            </button>
          </div>
        )}
        ...
        {showUrlDropdown && (
          <div className="absolute top-full left-4 right-4 mt-2 bg-surface-100/95 backdrop-blur-xl border border-surface-200/80 rounded-2xl p-5 shadow-2xl z-50 animate-fade-in max-w-sm mx-auto text-left">
            <div className="flex items-center gap-2 mb-3">
              <div className="p-1 bg-brand/10 text-brand rounded-md">
                <Link2 size={14} />
              </div>
              <h3 className="text-xs font-bold text-white uppercase tracking-wider">Play Stream from URL</h3>
            </div>
            ...
            <form onSubmit={handlePlayStream} className="flex flex-col gap-2">
              <div className="relative">
                <input
                  ...
                  className="w-full h-8 pl-3 pr-14 rounded-lg bg-surface-200 border border-surface-300
                ...
                <button
                  ...
                  className="absolute right-1.5 top-1/2 -translate-y-1/2 px-1.5 py-0.5 rounded
                             text-[10px] bg-surface-300 hover:bg-surface-300/80 text-gray-300 transition-all border border-white/5"
                >
                  Paste
                </button>
              </div>
